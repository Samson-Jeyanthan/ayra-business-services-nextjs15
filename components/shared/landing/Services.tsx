...
const Services = () => {
  return (
    <section className="flex gap-4 min-h-[85vh] bg-light-800 p-20">
      <div className="w-1/2 flex flex-col gap-8">
        <h2 className="heading-2">Services We Provide</h2>
        <p className="font-medium text-light-500">
        ...
        >
          {SERVICES.map((item, index) => (
            <AccordionItem value={item.value} key={index} className="py-2">
              <div className="border-t-2 border-light-300 border-solid mb-2" />
              <AccordionTrigger className="text-2xl font-semibold hover:text-decoration-none cursor-pointer">
                {item.name}
              </AccordionTrigger>
              <AccordionContent className="text-light-500 text-base w-[92%]">
                {item.description}
              </AccordionContent>
            </AccordionItem>
          ))}
